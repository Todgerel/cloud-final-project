...
export default function ContactForm() {
  ...
  async function handleSubmit(event) {
    ...
    setIsValidated(true);
    const { name, email, message, subject, phone } = form.elements;
    const data = {
      name: name.value,
      email: email.value,
    ...
    if (form.checkValidity()) {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `${response.status} ${response.statusText}, check backendUrl in data.js`
          );
        }
  ...
  return (
    <>
      <Form noValidate validated={isValidated} onSubmit={handleSubmit}>
        ...
        <Form.Group className="mx-auto mb-3 form-group" controlId="phone">
        ...
        <Form.Group className="mx-auto mb-3 form-group" controlId="subject">
          ...
          <Form.Control.Feedback type="invalid">
            <h5>Subject must be at least one character.</h5>
          </Form.Control.Feedback>
        </Form.Group>
        ...
        <Form.Group className="mx-auto text-center form-group">
          ...
          <Alert show={!backendUrl} variant="danger">
            <Alert.Heading>
              You must provide a valid backend url in data.js
            </Alert.Heading>
          </Alert>
        </Form.Group>
      </Form>
